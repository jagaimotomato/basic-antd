...
var (
	...
	ErrFailedUsername = errors.New("用户不存在")

	ErrFailedPassword = errors.New("密码错误")

	ErrFailedUserStatus = errors.New("该用户已被停用")
...
func (mw *GinJWTMiddleware) middlewareImpl(c *gin.Context) {
	...
	if int64(claims["exp"].(float64)) < mw.TimeFunc().Unix() {
		mw.unauthorized(c, 2887, mw.HTTPStatusMessageFunc(ErrExpiredToken, c))
		return
	}
