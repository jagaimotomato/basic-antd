...
func (p *Permission) GetList() (permissions []Permission, err error) {
	...
	if err = table.Order("sort").Find(&permissions).Error; err != nil {
		return
	}
	return
}
...
func (p *Permission) GetRecursionPermission(pageIndex, pageSize int) (permissions []Permission, count int64, err error) {
	...
	for i := 0; i < len(list); i++ {
		...
		children := RecursionAllPermission(&page, list[i])
...
func (p *Permission) GetAdminMenus() (menus []Permission, err error) {
	list, err := p.GetList()
	if err != nil {
		return
	}
